...
import { runReverbSupplySync } from './sync_reverb.js';
import { debugReverbOrder, debugReverbLabel, debugReverbListing } from './reverb_debug.js';
...
app.get('/health', (req, res) => {
  res.json({ ok: true, app: 'shipstation-supplies-sync', version: '1.0.6-reverb-category-sync', config: publicConfig() });
});
...
app.get('/jobs/reverb/supplies-sync', async (req, res) => {
  if (!checkSecret(req, res)) return;
  try {
    const dryRunOverride = req.query.dryRun === undefined ? null : ['1', 'true', 'yes', 'y'].includes(String(req.query.dryRun).toLowerCase());
    const result = await runReverbSupplySync({
      store: req.query.store || 'main',
      onlyOrder: req.query.order || '',
      dryRunOverride,
    });
    res.json(result);
  } catch (err) {
...
app.get('/jobs/reverb/debug-listing', async (req, res) => {
  if (!checkSecret(req, res)) return;
  try {
    const result = await debugReverbListing({ store: req.query.store || 'main', order: req.query.order, listing: req.query.listing });
    res.json(result);
  } catch (err) {
    res.status(err.status || 500).json({ ok: false, error: err.message, status: err.status, body: err.body });
  }
