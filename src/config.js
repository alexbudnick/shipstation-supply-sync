...
export const config = {
  ...
  debugPreview: bool('DEBUG_PREVIEW', true),
  ...
  reverbBaseUrl: (process.env.REVERB_BASE_URL || 'https://api.reverb.com/api').replace(/\/$/, ''),
  reverbMainToken: process.env.REVERB_MAIN_TOKEN || process.env.REVERB_TOKEN_MAIN || process.env.REVERB_API_TOKEN_MAIN || '',
  reverbWarehouseToken: process.env.REVERB_WAREHOUSE_TOKEN || process.env.REVERB_TOKEN_WAREHOUSE || process.env.REVERB_API_TOKEN_WAREHOUSE || '',
};
...
export function publicConfig() {
  return {
    ...
    debugPreview: config.debugPreview,
    ...
    reverbBaseUrl: config.reverbBaseUrl,
    hasReverbMainToken: Boolean(config.reverbMainToken),
    hasReverbWarehouseToken: Boolean(config.reverbWarehouseToken),
  };
}
