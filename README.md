...
## Reverb category supply sync

New endpoint:

```text
/jobs/reverb/supplies-sync?secret=YOUR_SECRET&store=main&dryRun=true
```

Single-order test:

```text
/jobs/reverb/supplies-sync?secret=YOUR_SECRET&store=main&order=26226710&dryRun=true
```

Rules:

- Electric Guitars -> Box P
- Acoustic, hollow, semi-hollow, archtop -> Box Q
- Bass / basses -> Box R

The sync first checks for existing Supply Movements by Movement ID, Reverb Order ID, Order Number, or Tracking Number. If ShipStation already created a Needs Review row for the same order/tracking, Reverb sync updates that row instead of creating a duplicate.
